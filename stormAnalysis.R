...
myStormData$YEAR<- format(as.POSIXct(myStormData$BGN_DATE, 
                                          format = "%m/%d/%Y %H:%M:%S", tz = "" ), "%Y")
...
#print(head(myStorms))
#myDeadlyStorms <- myStorms$stormType[1:5]

#myStormData <- subset(myStormData, myStormData$EVTYPE %in% myDeadlyStorms)
#print(myStormData)
#myStorms_summary <- myStormData %>%
#  group_by(YEAR, EVTYPE) %>%
#  summarize(Sum_Value = sum(FATALITIES), .groups = "drop") # Calculate the sum
#View(myStorms_summary)
...
myStorms_summary <- myStormData %>%
  ...
  testPlot <- ggplot(myStorms_summary, aes(x = YEAR, y = Sum_Value, fill = EVTYPE)) +
    geom_bar(stat = "identity", position = "dodge") +
    facet_wrap(~ EVTYPE, ncol = 1, scales="free_y")
    labs(title = "Sum of Fatalities by Year (Top 5 EVTYPEs)",
         x = "Year",
         y = "Sum of Fatalities",
         fill = "Event Type") +  # More descriptive fill label
    theme_bw() +
